...
Write-LogFile -Message "Install the Microsoft Azure Virtual Desktop WebRTC Redirector Service."
...
try {
    $OutFile = Get-EvergreenApp -Name "MicrosoftWvdRtcService" | `
    ...
    $OutFile = Get-EvergreenAppFromApi -Name "MicrosoftWvdRtcService" | `
        Where-Object { $_.Architecture -eq "x64" } | `
        Select-Object -First 1 | `
        Save-EvergreenApp -CustomPath $Path -ErrorAction "Stop"
}
