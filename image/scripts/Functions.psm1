...
function Write-LogFile {
    ...
    begin {
        # Log file path
        $LogFile = "$Env:SystemRoot\Logs\ImageBuild\ImageBuild-$(Get-Date -Format "yyyy-MM-dd").log"
        if (!(Test-Path -Path "$Env:SystemRoot\Logs\ImageBuild")) {
            New-Item -Path "$Env:SystemRoot\Logs\ImageBuild" -ItemType "Directory" -ErrorAction "SilentlyContinue" | Out-Null
            $params = @{
...
function Get-InstallConfig {
    ...
    Write-LogFile -Message "Read package install config: $Uri"
    try {
        $params = @{
            Uri             = $Uri
